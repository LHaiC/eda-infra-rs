// test_helper.cu
#include "dcsr/dcsr_view.h"
#include "dcsr/dcsr_view_helpers.cuh" 
#include "utils/cuda-utils.hpp"

__global__ void verify_sum_kernel(DcsrView view, int* d_results) {
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= view.num_nodes) return;
    int sum = 0;
    for (auto val : dcsr::neighbors<int>(view, tid)) {
        sum += val;
    }
    d_results[tid] = sum;
}

__global__ void verify_naive_csr_sum_kernel(
    const uint32_t* indptr, 
    const int* data, 
    int* d_results, 
    uint32_t num_nodes
) {
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= num_nodes) return;

    int sum = 0;
    uint32_t start = indptr[tid];
    uint32_t end = indptr[tid + 1];

    for (uint32_t i = start; i < end; ++i) {
        sum += data[i];
    }
    d_results[tid] = sum;
}

extern "C" {
    void dcsr_test_verify_sum(DcsrView view, int* host_results, uint32_t count) {
        int* d_results = (int *)cudaMallocCheck(count * sizeof(int));
        
        uint32_t threads = 256;
        uint32_t blocks = (count + threads - 1) / threads;
        
        verify_sum_kernel<<<blocks, threads>>>(view, d_results);
        cudaSyncCheck();
        
        cudaMemcpyD2HCheck(host_results, d_results, count * sizeof(int));
        cudaFreeCheck(d_results);
    }

    void naive_csr_test_verify_sum(
        const uint32_t* d_indptr, 
        const int* d_data, 
        int* host_results, 
        uint32_t count
    ) {
        int* d_results = (int *)cudaMallocCheck(count * sizeof(int));
        
        uint32_t threads = 256;
        uint32_t blocks = (count + threads - 1) / threads;
        
        verify_naive_csr_sum_kernel<<<blocks, threads>>>(d_indptr, d_data, d_results, count);
        cudaSyncCheck();
        
        cudaMemcpyD2HCheck(host_results, d_results, count * sizeof(int));
        cudaFreeCheck(d_results);
    }
}